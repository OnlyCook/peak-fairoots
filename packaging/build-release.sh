...
# script syncs it into Fairoots.csproj and PluginInfo.cs before building
# (CHANGELOG.md stays hand-maintained, this only warns if the new version has
# no entry yet).
#
# Output: dist/Fairoots-<version>.zip with everything at the zip ROOT:
#   manifest.json
#   icon.png            (256x256)
#   README.md
#   CHANGELOG.md
...
#   Fairoots.dll
#
# r2modman installs the whole package into BepInEx/plugins/<Team>-Fairoots/,
# so a root-level DLL lands correctly and BepInEx loads it.
#
# Also writes a Nexus Mods distribution to
# dist/nexus/Fairoots-<version>-nexus.zip, which is the same file set as the
# Thunderstore zip but nested one level under an OnlyCook-Fairoots/ folder, so
# extracting it straight into BepInEx/plugins/ produces the correct layout
...
PROJ="$REPO_ROOT/src/Fairoots"
DIST="$REPO_ROOT/dist"
NEXUS_FOLDER="OnlyCook-Fairoots"
...
echo "Packaging Fairoots v$VERSION"
...
echo "Building..."
dotnet build "$PROJ/Fairoots.csproj" -c Release >/dev/null
DLL="$PROJ/bin/Release/Fairoots.dll"
[[ -f "$DLL" ]] || { echo "ERROR: build output not found: $DLL" >&2; exit 1; }

# 2. Validate the icon is exactly 256x256 (Thunderstore requirement).
if command -v python3 >/dev/null 2>&1 && [[ -f "$PKG/icon.png" ]]; then
...
cp "$PKG/manifest.json" "$PKG/README.md" "$PKG/CHANGELOG.md" "$STAGE/"
[[ -f "$PKG/icon.png" ]] && cp "$PKG/icon.png" "$STAGE/" || echo "NOTE: no packaging/icon.png yet (required, 256x256, before publishing)."
[[ -f "$REPO_ROOT/LICENSE" ]] && cp "$REPO_ROOT/LICENSE" "$STAGE/LICENSE" || echo "NOTE: no LICENSE file yet (pick one before publishing)."
cp "$DLL" "$STAGE/Fairoots.dll"

# 4. Zip (files at the root of the archive).
mkdir -p "$DIST"
OUT="$DIST/Fairoots-$VERSION.zip"
rm -f "$OUT"
...
NEXUS_OUT="$NEXUS_DIST/Fairoots-$VERSION-nexus.zip"
rm -f "$NEXUS_OUT"
